                               ...
                               uint32_t *inference_ms)
{
  ...
  dbox_to_roi(dbox, &src_roi, STAI_OD_IN_1_WIDTH, STAI_OD_IN_1_HEIGHT);
  STM32Ipl_Init(&src, STAI_OD_IN_1_WIDTH, STAI_OD_IN_1_HEIGHT, IMAGE_BPP_RGB888, src_data);
...
static void Display_Draw_TrackObject(TrackObject_s *o)
{
  ...
  uint32_t x0 = (uint32_t) ((dbox->x_center - dbox->width / 2) * ((float32_t) lcd_bg_area.XSize)) + lcd_bg_area.X0;
...
static void Display_WelcomeScreen(void)
{
  ...
  if (HAL_GetTick() - t0 < 4000)
  {
    ...
    UTIL_LCD_SetBackColor(0x40000000);
    UTIL_LCDEx_PrintfAt(0, LINE(16), CENTER_MODE, "Re-Identification");
